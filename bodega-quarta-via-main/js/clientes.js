document.addEventListener('DOMContentLoaded', function () {
    ...
    const erroObservacao = document.getElementById('erro-observacao');
    ...
    form.addEventListener('submit', function (e) {
        ...
        if (nomeValido.length < 3 || !/^[A-Za-zÀ-ÿ\s]+$/.test(nomeValido)) {
        ...
        if (telefoneNumeros.length < 10) {
            erroTelefone.textContent = "O telefone deve conter pelo menos 10 números.";
        ...
        const enderecoValido = endereco.value.trim();
        if (enderecoValido.length < 8) {
            erroEndereco.textContent = "O endereço deve conter pelo menos 5 caracteres.";
        ...
        // Validação da observação (opcional, mas se preenchida, mínimo 10 caracteres)
        const observacaoValida = observacao.value.trim();
        if (observacaoValida.length > 0 && observacaoValida.length < 10) {
            erroObservacao.textContent = "Se preenchida, a observação deve conter pelo menos 10 caracteres.";
            erroObservacao.style.display = 'block';
            observacao.classList.add('is-invalid');
            erros.push("observacao");
        } else {
            erroObservacao.textContent = "";
            erroObservacao.style.display = 'none';
        ...
        if (erros.length === 0) {
            ...
            // Remove erros visuais depois do reset
            erroNome.style.display = 'none';
            nome.classList.remove('is-invalid');
